...
summary(lm(colMeans(databin[winter,,1])~colMeans(databin[winter,,3])+colMeans(databin[winter,,2])))
...
if(model=='ed'){
p1.ed<-ggplot(gdat.w, aes(x=wlai,y=walb))+geom_point(size=0.4)+stat_smooth(method="lm", col='cyan', fill='cyan')+xlim(-1.3,2.8)+ylim(-0.06,0.075)+theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("ED2")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}
if(model=='jules'){
p1.jules<-ggplot(gdat.w, aes(x=wlai,y=walb))+geom_point(size=0.4)+stat_smooth(method="lm", col='cyan', fill='cyan')+xlim(-1.3,2.8)+ylim(-0.06,0.075)+theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("JULES")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}

gdat.s<-data.frame(cbind(colMeans(databin[summer,,1], na.rm=TRUE),colMeans(databin[summer,,2]))); colnames(gdat.s)<-c("salb",'slai')
if(model=='ed'){
p2.ed<-ggplot(gdat.s, aes(x=slai,y=salb))+geom_point(size=0.4)+stat_smooth(method="lm", col='seagreen', fill='seagreen')+xlim(-1.3,2.8)+ylim(-0.06,0.075)+ theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("ED2")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}
if(model=='jules'){
p2.jules<-ggplot(gdat.s, aes(x=slai,y=salb))+geom_point(size=0.4)+stat_smooth(method="lm", col='seagreen', fill='seagreen')+xlim(-1.3,2.8)+ylim(-0.06,0.075)+ theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("JULES")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}


p1e<-readRDS("p1e.rds");p2e<-readRDS("p2e.rds")
