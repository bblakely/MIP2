...
model<-'ed'
if(model=='jules'){databin<-(-databin.jules[,c(1:5, 7:12),])}; if(model=='ed'){databin<-(-databin.ed)} #;databin<-databin[,c(1:3,5),]

allplots=FALSE
if(allplots==TRUE){
#Plot snow change in Jan
par(mfrow=c(1,1)) 
plot(databin[,1,3], col='white', yli m=c(min(databin[,,3]),max(databin[,,3])))
...
if(model=='ed'){
p2.ed<-ggplot(gdat.s, aes(x=slai,y=salb))+geom_point(size=0.4)+stat_smooth(method="lm", col='seagreen', fill='seagreen')+xlim(-2.5,2.8)+ylim(-0.07,0.075)+ theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("ED2")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}
if(model=='jules'){
p2.jules<-ggplot(gdat.s, aes(x=slai,y=salb))+geom_point(size=0.4)+stat_smooth(method="lm", col='seagreen', fill='seagreen')+xlim(-2.5,2.8)+ylim(-0.07,0.075)+ theme_bw()+ylab(albexp)+xlab(laiexp)+ggtitle("JULES")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}
...
if(exists("p2.jules") & exists("p2.ed")){
  ...
  dev.copy(png, filename="Figures/LAIsens.png", width=800, height=420);dev.off()
}
...
if(model=='ed'){
  p2s.ed<-ggplot(gdat.ws, aes(x=wsno,y=walb))+geom_point(size=0.4)+stat_smooth(method="lm", col='cyan', fill='cyan')+xlim(-30,15)+ylim(-0.07,0.075)+ theme_bw()+ylab(albexp)+xlab(snoexp)+ggtitle("ED2")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}
if(model=='jules'){
  p2s.jules<-ggplot(gdat.ws, aes(x=wsno,y=walb))+geom_point(size=0.4)+stat_smooth(method="lm", col='cyan', fill='cyan')+xlim(-30,15)+ylim(-0.07,0.075)+ theme_bw()+ylab(albexp)+xlab(snoexp)+ggtitle("JULES")+geom_hline(yintercept=0, linetype="dashed", size=0.2)
}

if(exists("p2s.jules") & exists("p2s.ed")){
  grid.arrange(p2s.ed,p2s.jules, ncol=2)
  dev.copy(png, filename="Figures/SNOsens.png", width=533, height=220);dev.off()
}
...
mean(colMeans(databin[c(6:8),,1]))#JJA
...
par(mfrow=c(1,2))
#plot(colMeans(-databin.ed[summer,,1])~colMeans(-databin.ed[summer,,5]), ylim=c(-0.01, 0.05), xlim=c(0, 0.5)); abline(h=0)
#plot(colMeans(-databin.jules[summer,,1])~colMeans(-databin.jules[summer,,5]), ylim=c(-0.01, 0.05), xlim=c(0, 0.5));abline(h=0)

mean(colMeans(-databin.ed[summer,,5]));
mean(colMeans(-databin.jules[summer,c(1:5, 7:12),5]));

par(mfrow=c(2,1))
#hist(colMeans(-databin.ed[summer,,5]), xlim=c(0.05, 0.35), main='ed')
#hist(colMeans(-databin.jules[summer,c(1:5, 7:12),5]), xlim=c(0.05, 0.35), main='jules')

summary(lm((colMeans(-databin.ed[summer,,1])~colMeans(-databin.ed[summer,,5]))))
summary(lm((colMeans(-databin.jules[summer,c(1:5, 7:12),1])~colMeans(-databin.jules[summer,c(1:5, 7:12),5]))))
